...
# Bài Thực Hành 8: Amazon DynamoDB Streams và AWS Lambda

Sư kết hợp giữa DynamoDB Stream với AWS Lambda tạo ra nhiều mẫu kiến trúc mới vô củng mạnh mẽ. Trong bài thực hành này, bạn sẽ sao chép các item từ bảng DynamoDB sang một bảng khác sử dụng DynamoDB Stream và AWS Lambda. 
DynamoDB Stream chụp lại một chuỗi sự thay đổi ở mức item và xếp lại theo thứ tự thời gian rồi lưu vào một nhật ký có thời hạn lưu trữ là 24h. Mọi ứng dụng đều có thể truy cập và xem dữ liệu nhật kí này với thời gian chỉnh sửa so với thực tế là gân thực. 
DynamoDB stream được sử dụng trong các trường hợp sau đây: 
...
Chúng ta sẽ sử dụng lại bảng **logfile**  mà chúng ta đã tạo ở bài thực hành số 1 để bật DynamoDB Stream cho nó. Bất cứ khi nào có sự thay đổi trên bảng **logfile**, ngay lập tức thông tin thay đổi sẽ được đưa vào stream. 
Tiếp theo ta sẽ gắn một Lambda function vào stream với mục đích truy vấn sự thay đổi trên bảng **logfile**, rồi ghi lại những cập nhật đó vào bảng được tạo mới có tên là **logfile_replica**. 
Biểu đồ sau thể hiện các bước thực hiện của bài thực hành này: 
![Security Hub](../../../images/23.png?width=60pc)  

### Bước 1 - Tạo Bảng Sao chép
...
Chạy lệnh sau, đợi cho tới khi trạng thái của bảng chuyển thành **ACTIVE**
```python
aws dynamodb wait table-exists --table-name logfile_replica
```

### Bước 2 - Kiểm tra lại IAM Policy gán cho AWS Lambda
...
```
Lấy thông tin ARN của IAM Role mã sẽ được gán cho Lambda function. 
```python
...
```
Sao chép ARN có được ở trên, thay thế cho ***YOUR_ARN_HERE*** rồi chạy lệnh tạo Lambda function
```python
...
```
Lấy thông tin của DynamoDB Stream
```python
...
### Bước 5 - Nối nguồn Stream với Lambda function
...
### Bước 6 - Đẩy dữ liệu vào bảng logfile và kiểm tra tiến trình sao chép vào bảng logfile_replica
...
Để kiểm tra tiến trình sao chép vào bảng **logfile_replica** có thành công hay chưa, thực hiện quét dữ liệu bảng **logfile_replica**. 
```python
...
# CHÚC MỪNG BẠN!!! BẠN ĐÃ HOÀN THÀNH TẤT CẢ CÁC BÀI THỰC HÀNH TRONG WORKSHOP NÀY!
